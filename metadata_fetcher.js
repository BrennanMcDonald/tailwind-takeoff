...
const scraper = function (URL) {
  axios.get(URL).then(res => {
    ...
    var classes = tableParser.getElementsByTagName('tr').map(el => el.childNodes[1].textContent).filter(el => el.startsWith('.'));
    var resultJSON = {
      title,
      classes,
    ...
    var safeTitle = title.replace(/^(.*)v(.*)+(.*)$/gm, '').trim().toLowerCase().replace(/\s\/\s/gm, '-').replace(/\s/gm, '_')
    fs.writeFile(`metadata/${safeTitle}.json`, JSON.stringify(resultJSON), (err) => {
      if (err) throw err;
...
axios.get(URL).then(res => {
  var parser = new DOMParser();
  var htmlDoc = parser.parseFromString(res.data, 'text/html');
  var listParser = parser.parseFromString(htmlDoc.getElementById('nav').outerHTML, 'text/html');
  ...
  output = output.map(el => {
    ...
    return listParser.getElementsByTagName('a').map(a => `https://tailwindcss.com${a.getAttribute('href')}`);
  });
